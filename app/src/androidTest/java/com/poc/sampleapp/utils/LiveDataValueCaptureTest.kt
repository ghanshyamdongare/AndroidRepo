...
class LiveDataValueCaptureTest<T> {
 ...
 */
inline fun <T> LiveData<T>.captureValues(block: LiveDataValueCaptureTest<T>.() -> Unit) {
    val capture = LiveDataValueCaptureTest<T>()
 ...
 */
fun <T> LiveData<T>.getValueForTestLiveData(): T? {
    var value: T? = null
